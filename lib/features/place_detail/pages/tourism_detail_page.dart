...
class TourismDetailPage extends StatelessWidget {
  const TourismDetailPage({super.key, this.data = attractionDetailMockData});

  final TourismDetailData data;
  ...
  List<FacilityItem> get _tourismFacilities {
    const allowedTypes = {
  ...
  bool get _hasExperienceInfo =>
      _hasValue(data.experienceAge) || _hasValue(data.experienceGuide);

  bool get _hasUsageInfo =>
      _hasValue(data.dayOff) ||
      _hasValue(data.admissionFee) ||
      _hasValue(data.openPeriod) ||
      _hasValue(data.useTime) ||
      _hasValue(data.capacity) ||
      _hasValue(data.parkingFee);

  bool get _hasContactInfo =>
      _hasValue(data.contact) ||
      _hasValue(data.reservationInfo) ||
                      ...
                      child: SizedBox(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_hasExperienceInfo) ...[
                              const SizedBox(height: 26),
                              _buildExperienceInfo(),
                            ],

                            if (_tourismFacilities.isNotEmpty) ...[
                              const SizedBox(height: 26),
                              const _DetailDivider(),
  ...
  Widget _buildExperienceInfo() {
    final values = <String>[
      if (_hasValue(data.experienceAge)) data.experienceAge!,
      if (_hasValue(data.experienceGuide)) data.experienceGuide!,
    ];
